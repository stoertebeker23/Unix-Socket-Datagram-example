...
void cleanup() {
	...
	char *cli = (char*)malloc (strlen(CLIENT) -1 + strlen(username));
	strcpy(cli,CLIENT);
	strcat(cli,username);
...
void output_handler(const char buffer[], int line, struct winsize * size) {
	...
	printf("\033[%d;0H--------------------------------------- %d,%d",line+2, size->ws_row, size->ws_col);
...
int main (int argc, char* argv[]) {
	...
	// Create socket path
	char *cli = (char*)malloc (strlen(CLIENT) -1 + strlen(argv[1]));
	strcat(cli,CLIENT);
	strcat(cli,argv[1]);
